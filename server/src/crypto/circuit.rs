...
use crate::crypto::asset::AssetHash;
use crate::crypto::id::{id_commitment, Address, NullifierKey};
use crate::crypto::note::{BlindNoteHash, NoteHistory, NoteOutIndex};
use crate::crypto::tx::{IssueTx, SealedIssueTx, SealedSplitTx, SplitTx, Tx};
use crate::crypto::Error;
use crate::crypto::FWrap;
...
fn verify_step<E: SNARK>(
    ...
    impl Sat {
        ...
        fn result(&self) -> Result<(), Error> {
            self.inner
                .clone()
                .map_err(|_| Error::With("invalid witness"))
        }
    }
